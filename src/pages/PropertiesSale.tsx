...
const saleProperties = mockProperties.filter((p) => p.category === 'sale');
...
export default function PropertiesSale() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === 'area-asc') data.sort((a, b) => a.area - b.area);
    return data;
  }, [sort, search, selectedBeds]);
      ...
      <main className="pt-24">
        ...
        <div className="max-w-[1280px] mx-auto px-5 md:px-16 py-12">
          ...
          <p className="text-[16px] text-secondary mb-8">
            Hiển thị{' '}
            <span className="font-semibold text-primary">{filtered.length}</span> bất động sản
          </p>
